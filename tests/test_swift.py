...
def test_swift_mirror_downloads_all_objects_and_skips_unchanged(tmp_path):
    ...
    assert (
        tmp_path / "artifacts" / uuid / "generated/github-runner/jobs.json"
    ).read_text() == "world"
    ...
    assert events == [
        {
            ...
            "total": 1,
            "name": f"{uuid}/generated/sunbeam/output.log",
            "path": str(tmp_path / "artifacts" / uuid / "generated/sunbeam/output.log"),
            "url": output_url,
            "status": "downloading",
        },
        {
...
def test_swift_mirror_download_error_names_object_url_and_path(tmp_path):
    ...
    http = FakeHttp({
        index_url: index,
        output_url: HTTPError(output_url, 404, "Not Found", Message(), None),
    })
    config = Config.load(None)
    config.swift.base_url = base
    config.paths.artifact_root = tmp_path / "artifacts"
    ...
    assert (
        f"Failed to download Swift object {uuid}/generated/sunbeam/output.log"
        in message
    )
    assert output_url in message
    assert (
        str(tmp_path / "artifacts" / uuid / "generated/sunbeam/output.log") in message
    )
...
def test_swift_mirror_can_continue_after_download_errors(tmp_path):
    ...
    http = FakeHttp({
        index_url: index,
        missing_url: HTTPError(missing_url, 404, "Not Found", Message(), None),
        jobs_url: b"world",
    })
    config = Config.load(None)
    config.swift.base_url = base
...
def test_swift_mirror_raises_clear_error_for_missing_uuid(tmp_path):
    ...
    index_url = f"{base}/{uuid}/index.html"
    http = FakeHttp({
        index_url: HTTPError(index_url, 404, "Not Found", Message(), None),
    })
...
def test_swift_mirror_uses_index_html_links_and_ignores_unsafe_links(tmp_path):
    ...
    manifest = SwiftMirror(
        config.swift, config.paths.artifact_root, http=http
    ).mirror_uuid(uuid)
    ...
    assert (
        tmp_path / "artifacts" / uuid / "generated/sunbeam/output one.log"
    ).read_text() == "hello"
